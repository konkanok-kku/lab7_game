...
func perform_jump():
	...
	jumpTween()
	animation.play("Armature|mixamo_com|Layer0_001")
	velocity.y = jump_force

func perform_flip_jump():
	AudioManager.jump_sfx.play()
	AudioManager.jump_sfx.pitch_scale = 0.8
	animation.play("Armature|mixamo_com|Layer0_001", -1, 2)
	velocity.y = jump_force
	await animation.animation_finished
	can_double_jump = false
	animation.play("Armature|mixamo_com|Layer0_001", 0.5)
...
func player_animations():
	...
	if is_on_floor():
		if is_moving(): # Checks if player is moving
			...
			animation.play("Armature|mixamo_com|Layer0", 0.5)
